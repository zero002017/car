...
@ccclass
export class MeiriGems extends Popup {
	...
	private refresh(): void {
		...
		this.watchVideo.set("现在观看", "每日钻石", DatasManager.每日钻石观看视频, null, () => {
			main.moneys.addMoney("钻石", this.currNum, this.icon, null);
			UserStorage.meiriGemsTimes++;
			this.refresh();
			UserStorage.flush();
		});

		let currTimes: number = UserStorage.meiriGemsTimes + 1;

		let nextBigTimes: number;
		if (currTimes < DatasManager.第n次每日钻石s.length - 1) {
			nextBigTimes = currTimes;
			while (nextBigTimes < DatasManager.第n次每日钻石s.length) {
				if (DatasManager.第n次每日钻石s[nextBigTimes]) {
					break;
				}
				nextBigTimes++;
			}
		} else {
			nextBigTimes = DatasManager.第n次每日钻石s.length - 1;
		}

		this.times2.string = nextBigTimes.toString();
		this.num2.string = "x" + DatasManager.第n次每日钻石s[nextBigTimes];

		this.times.string = currTimes.toString();
		this.currNum = DatasManager.第n次每日钻石s[currTimes] || DatasManager.第n次每日钻石;
