...
export default class PlatformCtrlWX extends PlatformCtrl {
    ...
    private bannerAds: Array<WXBannerAdContainer>;
    private currBannerAdContainer: WXBannerAdContainer;

    private insertAdIndex: number;
    private insertAds: Array<WXInsertAdContainer>;

    private gameBannerAdIndex: number;
    private gameBannerAds: Array<WXGameBannerAdContainer>;
    private currGameBannerAdContainer: WXGameBannerAdContainer;

    private gamePortalAdIndex: number;
    private gamePortalAds: Array<WXGamePortalAdContainer>;

    private gridAdIndex: number;
    private gridAds: Array<WXGridAdContainer>;
    private currGridAdContainer: WXGridAdContainer;

    private customAdIndex: number;
    private customAds: Array<WXCustomAdContainer>;
    private currCustomAdContainer: WXCustomAdContainer;
    ...
    private loadNextBannerAd(): void {
        if (wx.createBannerAd) {
            if (this.bannerAds.length < DatasManager.微信Banner广告ID们.length) {
                this.bannerAds.push(new WXBannerAdContainer(DatasManager.微信Banner广告ID们[this.bannerAds.length]));
            }
        }
    }
    ...
    private loadNextInsertAd(): void {
        if (wx.createInterstitialAd) {
            if (this.insertAds.length < DatasManager.微信插屏广告ID们.length) {
                this.insertAds.push(new WXInsertAdContainer(DatasManager.微信插屏广告ID们[this.insertAds.length]));
            }
        }
    }
    ...
    private loadNextGameBannerAd(): void {
        if (wx.createGameBanner) {
            if (this.gameBannerAds.length < DatasManager.GameBannerID们.length) {
                this.gameBannerAds.push(new WXGameBannerAdContainer(DatasManager.GameBannerID们[this.gameBannerAds.length]));
            }
        }
    }
    ...
    private loadNextGamePortalAd(): void {
        if (wx.createGamePortal) {
            if (this.gamePortalAds.length < DatasManager.GamePortalID们.length) {
                this.gamePortalAds.push(new WXGamePortalAdContainer(DatasManager.GamePortalID们[this.gamePortalAds.length]));
            }
        }
    }
    ...
    private loadNextGridAd(): void {
        if (wx.createGridAd) {
            if (this.gridAds.length < DatasManager.微信格子广告ID们.length) {
                this.gridAds.push(new WXGridAdContainer(DatasManager.微信格子广告ID们[this.gridAds.length]));
            }
        }
    }
    ...
    private loadNextCustomAd(): void {
        if (wx.createCustomAd) {
            if (this.customAds.length < DatasManager.微信原生广告ID们.length) {
                this.customAds.push(new WXCustomAdContainer(DatasManager.微信原生广告ID们[this.customAds.length]));
            }
        }
    }
    ...
    //这里不能用this
    private _rewardAdLoad(): void { (platformCtrl as PlatformCtrlWX).rewardAdLoad(); }

    //这里不能用this
    private _rewardAdClose(res): void { (platformCtrl as PlatformCtrlWX).rewardAdClose(res); }

    //这里不能用this
    private _rewardAdError(): void { (platformCtrl as PlatformCtrlWX).rewardAdError(); }
